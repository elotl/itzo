...
import (
	...
	"github.com/justnoise/genny/parse"
...
func TestParse(t *testing.T) {
	...
	for _, test := range tests {
		...
		// assert the response
		msg := fmt.Sprintf("Parse didn't generate the expected output: %s", test.filename)
		if !assert.Equal(t, string(bytes), test.expectedOut, msg) {
